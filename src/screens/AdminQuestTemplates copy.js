...
const AdminQuestTemplates = ({ navigation }) => {
  ...
  // Special validation for location check-in
  if (newTemplate.type === 'location_checkin') {
    if (!newTemplate.locationName || newTemplate.locationName.trim() === '') {
      Alert.alert('Error', 'Please enter location name for check-in template');
      return;
    }
    if (!newTemplate.address || newTemplate.address.trim() === '') {
      Alert.alert('Error', 'Please enter address for check-in template');
      return;
    }
  }
  ...
  try {
    ...
    // Add location data only for location check-in type
    if (newTemplate.type === 'location_checkin') {
      templateData.requiredData = {
        locationName: newTemplate.locationName.trim(),
        address: newTemplate.address.trim(),
        coordinates: newTemplate.coordinates.trim() || '13.7563,100.5018', // Default to Bangkok
    ...
    try {
      ...
      Alert.alert('Success', 'Quest template created successfully!');
    } catch (apiError) {
      ...
      Alert.alert('Success', 'Quest template created successfully! (Mock data)');
    }
  } catch (error) {
  ...
  const handleUpdateTemplate = async () => {
    ...
    try {
      ...
      const templateData = {
        ...
        requiredData: newTemplate.type === 'location_checkin' ? {
    ...
    <View style={styles.container}>
      ...
      {/* <TouchableOpacity style={styles.header} onPress={handleProfilePress}>
        ...
        <View style={styles.headerText}>
          ...
          <Text style={styles.userRole}>Templates Management</Text>
        </View>
